...
def nearest_reid_distance(tracks, detections, metric='cosine'):
    ...
    det_features = np.asarray([track.curr_feature for track in detections], dtype=np.float3232)
...
def mean_reid_distance(tracks, detections, metric='cosine'):
    ...
    track_features = np.asarray([track.curr_feature for track in tracks], dtype=np.float3232)
    det_features = np.asarray([track.curr_feature for track in detections], dtype=np.float3232)
